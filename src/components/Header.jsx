...
const Header = () => {
  ...
  const domesticTourRegions = [
    { name: "Bakı", path: "/tours/baku", subTours: [
      { name: "İçərişəhər və Qız Qalası Turu", path: "/tours/domestic/baku/icherisheher-qiz-qalasi" },
      { name: "Müasir Bakı və Memarlıq Turu", path: "/tours/domestic/baku/modern-baku-architecture" },
      { name: "Bakı Qastronomik Turu", path: "/tours/domestic/baku/baku-gastronomy" },
    ]},
    { name: "Bakı Ətrafı", path: "/tours/baku", subTours: [
      { name: "Abşeron Yarımadası Turu", path: "/tours/domestic/baku-surroundings/absheron-peninsula" },
      { name: "Qobustan və Palçıq Vulkanları Səyahəti", path: "/tours/domestic/baku-surroundings/gobustan-mud-volcanoes" },
      { name: "Xəzər Sahili və Abşeron Qalaları", path: "/tours/domestic/baku-surroundings/caspian-coast-castles" },
    ]},
            ...
            <div className="hidden lg:flex items-center">
              <nav className="flex items-center space-x-5">
                ...
                <div className="relative" onMouseEnter={() => handleMouseEnter('services')} onMouseLeave={handleMouseLeave}>
                  <button className="flex items-center text-gray-700 hover:text-blue-600 font-medium">Our Services <ChevronDown size={16} className="ml-1" /></button>
                  {activeMenu === 'services' && (
                    <div className="absolute top-full left-0 mt-2 w-60 bg-white rounded-lg shadow-xl py-2 z-50 animate-fade-in">
                      {serviceItems.map(item => (
                        <Link key={item.name} to={item.path} className="flex items-center space-x-3 px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600">{item.icon}<span>{item.name}</span></Link>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <div className="relative" onMouseEnter={() => handleMouseEnter('international')} onMouseLeave={handleMouseLeave}>
                  <Link to="/international/turkey" className="flex items-center text-gray-700 hover:text-blue-600 font-medium">International Tours <ChevronDown size={16} className="ml-1" /></Link>
                  {activeMenu === 'international' && (
                    <div className="absolute top-full left-0 mt-2 w-56 bg-white rounded-lg shadow-xl py-2 z-50 animate-fade-in">
                      {internationalTourItems.map(item => (
                        <Link key={item.name} to={item.path} className="block px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600">{item.name}</Link>
                      ))}
                    </div>
                  )}
                </div>
                ...
                <div className="relative" onMouseEnter={() => handleMouseEnter('domestic')} onMouseLeave={handleMouseLeave}>
                  <Link to="/tours/baku" className="flex items-center text-gray-700 hover:text-blue-600 font-medium">Domestic Tours <ChevronDown size={16} className="ml-1" /></Link>
                  {activeMenu === 'domestic' && (
                    <div className="absolute top-full left-0 mt-2 w-64 bg-white rounded-lg shadow-xl py-2 z-50 animate-fade-in">
                      {domesticTourRegions.map(region => (
                        <div key={region.name} className="relative group/submenu">
                          <Link to={region.path} className="flex justify-between items-center w-full px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600">
                            {region.name}
                          ...
                          {region.subTours && (
                            <div className="absolute top-0 left-full ml-1 w-56 bg-white rounded-lg shadow-xl py-2 hidden group-hover/submenu:block animate-fade-in"
                              ...
                              {region.subTours.map(subTour => (
                                <Link key={subTour.name} to={subTour.path} className="block px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600">{subTour.name}</Link>
                              ))}
                            </div>
                          )}
                        </div>
                ...
                <Link to="/services/huntingtour" className="text-gray-700 hover:text-blue-600 font-medium">Hunting Tour</Link>
                <Link to="/team" className="text-gray-700 hover:text-blue-600 font-medium">Our Team</Link>
              ...
              <div className="relative ml-6" onMouseEnter={() => handleMouseEnter('lang')} onMouseLeave={handleMouseLeave}>
                ...
                {activeMenu === 'lang' && (
                  <div className="absolute top-full right-0 mt-2 w-40 bg-white rounded-lg shadow-xl py-2 z-50 animate-fade-in">
                    {languages.map(lang => (
                      <a key={lang.code} href="#" onClick={(e) => { e.preventDefault(); setLanguage(lang.code); }} className="block px-4 py-2 text-gray-700 hover:bg-blue-50">{lang.name}</a>
                    ))}
                  </div>
                )}
              </div>
      ...
      <div className={`lg:hidden fixed inset-0 bg-white z-[65] transition-transform duration-500 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="pt-24 px-8 h-full overflow-y-auto">
          ...
          <div>
            ...
            {mobileSubMenu === 'domestic' && (
              <div className="pl-4 border-l-2 border-blue-200 space-y-1">
                {domesticTourRegions.map(region => (
                  <div key={region.name}>
                    <Link to={region.path} onClick={() => setIsMenuOpen(false)} className="block py-2 text-gray-600">{region.name}</Link>
                    {region.subTours && (
                      <div className="pl-4 border-l border-gray-300">
                        {region.subTours.map(subTour => (
                          <Link key={subTour.name} to={subTour.path} onClick={() => setIsMenuOpen(false)} className="block py-1 text-gray-500 text-sm">{subTour.name}</Link>
                        ))}
                      </div>
                    )}
                  </div>
          ...
          <Link to="/services/huntingtour" onClick={() => setIsMenuOpen(false)} className="block py-3 text-lg font-medium text-gray-800 hover:text-blue-600">Hunting Tour</Link>
          ...
          <div className="mt-8 border-t pt-4">
            ...
            {languages.map(lang => (
              <a key={lang.code} href="#" onClick={(e) => { e.preventDefault(); setLanguage(lang.code); setIsMenuOpen(false); }} className="block py-2 text-gray-800 hover:text-blue-600">{lang.name}</a>
            ))}
          </div>
        </nav>
      </div>
